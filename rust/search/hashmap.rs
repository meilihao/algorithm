// https://github.com/QMHTMY/RustBook/blob/main/code/chapter05/hashmap.rs
// hashmap.rs

// 用 slot 保存位置即key，data 保存数据
...
impl<T: Clone + PartialEq + Default> HashMap<T> {
    ...
    // 扩展开放寻址:
    // 发生冲突时不是顺序查找下一个开放槽，而是跳过若干个槽，从而更均匀地分散引起冲突的项
    // 跳过的大小必须使得表中的所有槽最终都能被访问. 为确保这一点，建议表大小是素数，这也为什么示例中要使用 11
    fn insert(&mut self, key: usize, value: T) {
        if 0 == key { panic!("Error: key must > 0"); } // 0用于保存T的默认值
        ...
        if 0 == self.slot[pos] { // 槽无数据直接插入
            ...
            while 0 != self.slot[next]
                && key != self.slot[next] { // 槽中已有数据且其key不等于输入的key
