// ref:
// - 基于B+ Tree的简单KV数据库: https://www.bilibili.com/video/BV16N411X7y8 https://gitee.com/mphyatyh/kvdb/
...
type BPNode struct {
	...
	Nodes  []*BPNode // 结点的子树（叶子结点的Nodes=nil）
	Items  []BPItem  // 叶子结点的数据记录（索引结点的Items=nil）
...
func (node *BPNode) setValue(key int64, value interface{}) {
	...
	num := len(node.Items)
	if num < 1 { // 该叶子节点原先没有数据
		node.Items = append(node.Items, item)
		node.MaxKey = item.Key
		return
	} else if key < node.Items[0].Key { // 比最小的数据还小
		node.Items = append([]BPItem{item}, node.Items...)
		return
	} else if key > node.Items[num-1].Key { // 比最大的数据还大
		node.Items = append(node.Items, item)
		node.MaxKey = item.Key
		return
	}
	...
	for i := 0; i < num; i++ {
		if node.Items[i].Key > key {
			...
			node.Items[i] = item
			return
		} else if node.Items[i].Key == key { // 覆盖
			node.Items[i] = item
			return
		}
	}
...
// 使用递归调用实现数据的插入: 插入时首先定位到叶子结点，然后调用 BPNode.setValue()来设置关键字
func (t *BPTree) setValue(parent *BPNode, node *BPNode, key int64, value interface{}) {
	for i := 0; i < len(node.Nodes); i++ {
		if key <= node.Nodes[i].MaxKey || i == len(node.Nodes)-1 { // len(node.Nodes)-1表示是最后一个子树了, 前面子树没匹配到, 只能放在这里了
